"""
heart_disease_form.py

Author: Simon Correa Marin
GitHub: https://github.com/simon2326/Heart-disease

Defines `individual_tab()`, which:
  - injects CSS
  - displays a medical‐style form on the left
  - shows a “Medical History Summary” + live prediction on the right
"""
import os
...
def individual_tab() -> None:
    """
    Renders the “Individual Prediction” tab:
   - inject CSS
   - show banner
   - left: form inputs
   - right: medical‐style summary + live LR prediction
    """
    # --- Inject CSS (must be inside a function, not at import time) ---
    st.markdown(
        """
        <style>
        ...
        /* Shorter, colored section‐boxes */
        .section-box {
            background-color: #e0f7fa;  /* light pastel blue */
            padding: 0.4rem 1rem;       /* reduce vertical padding */
            border-radius: 0.5rem;
            margin-bottom: 1rem;       /* slightly smaller margin */
            border: 1px solid #b2ebf2; /* complementary blue border */
        }
        </style>
        """,
    ...
    st.markdown(
        "This form uses a trained **Logistic Regression** model "
        "to estimate a patient’s likelihood of having heart disease."
    )
    st.markdown("---")

    # Two‐column layout: LEFT=form, RIGHT=summary + prediction
    ...
    with left_col:
        st.markdown(
            '<div class="section-heading">📝 Patient Medical Data</div>',
            unsafe_allow_html=True,
        )
        ...
        st.markdown('<div class="section-box">', unsafe_allow_html=True)
        st.markdown(
            '<div class="section-heading">❤️ Vital Signs</div>',
            unsafe_allow_html=True,
        )
    ...
    with right_col:
        st.markdown(
            '<div class="section-heading">📝 Medical History Summary</div>',
            unsafe_allow_html=True,
        )

        # Build a small HTML summary of the chosen inputs
        summary_html = f"""
        <div class="section-box">
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 0.5rem;">
            <div><span class="summary-label">Rest BP:</span> {rest_bp} mmHg</div>
        ...
        <div class="section-box">
          <div style="display: flex; justify-content: space-between;">
            <div><span class="summary-label">Max HR:</span> {max_hr} bpm</div>
        ...
        <div class="section-box">
          ...
          <div style="display: flex; justify-content: space-between; margin-top: 0.5rem;">
            ...
            <div><span class="summary-label">Major Vessels:</span> {ca}</div>
          </div>
